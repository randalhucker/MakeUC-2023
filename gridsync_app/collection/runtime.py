...
class Collection:
    ...
    def create_collection(self, csv_name: str) -> None:
        ...
        with open(f'/Users/randyhucker/Documents/GitHub/MakeUC-2023/gridsync_app/collection/{csv_name}.csv', 'r') as file:
    ...
    def convert_dict(self, input_dict):
        ...
        new_dict = {
            ...
            'load_requirement': round(load_requirement)  # Rounding to the nearest integer
        }
    ...
    def upload_model(self, model_name: str, model) -> None:
        if (self.collection.find_one({"_id": model_name}) == None):
            # Upload model to MongoDB
            self.collection.insert_one({"_id": model_name, "model": model})
        else:
            # Update model in MongoDB
            self.collection.update_one({"_id": model_name}, {"model": model})
